...
function render_stats(dataParsed) {
  var state =
    dataParsed.state.charAt(0).toUpperCase() + dataParsed.state.slice(1);
...
function render_ann_box(anns) {
  $("#anns_body").empty();
  $("#anns_body").append(
    '<tr class="flex" style="background: none;"> ' +
  ...
  for (ann in anns) {
    ...
    var new_row = $(
      '<tr class="flex">' +
        '<td class="flex-noShrink" style="width:110px;">' +
        date +
        "</td>" +
        '<td class="flex-noShrink" style="width:100px;">' +
        time +
        "</td>" +
        '<td class="flex-noShrink" style="width:180px; word-wrap:break-word;">' +
        poster +
        "</td>" +
        '<td class="flex-fillSpace">' +
        announcement +
        "</td> </tr>"
    );
    if (is_TA) {
      var del_ann_button = $(
        '<td><div align="right"><button class="btn btn-primary"><i class="fa fa-close" title="Delete"></i></button></div></td>'
      );
...
function render_ta_table(TAs) {
  ...
  if (TAs.length < 2) $("#tas_header").text("TA on Duty");
  else $("#tas_header").text("TAs on Duty");
  var TA;
  for (TA in TAs) {
  ...
  if (queue_state == "closed") {
    ...
    $("#state_button").click(function (event) {
      ...
      if (dataParsed.queue_length > 0) {
        var res = confirm(
          "Are you sure you want to close the queue? All students will be removed."
        );
        if (res) {
          close_queue(course_id);
        }
      } else close_queue(course_id);
    });
    ...
    if (!(my_username in dataParsed.TAs)) {
      //Checks if the TA is on duty
    ...
    document.getElementById("quest_public_checkbox").checked =
      dataParsed.quest_public;
    $("#quest_public_form").show();
    $("#quest_public_checkbox").click(function (event) {
      event.preventDefault();
      var quest_public = document.getElementById(
        "quest_public_checkbox"
      ).checked;
...
function render_student_view(dataParsed) {
  ...
  if (!in_queue) {
    ...
    $("#edit_question_button").hide();
    document.getElementById("join_button").className =
      "margin-top-5 btn btn-success";
...
function render_queue_table(dataParsed) {
  ...
  $("#queue_body").append(
    "<tr style='background: none;'>" +
      ...
      "<th class='col-sm-2' align='left' style='word-wrap: break-word'>" +
      location_heading +
      "</th>" +
      "<th class='col-sm-2' align='left' style='word-wrap: break-word'>" +
      question_heading +
  ...
  for (row in queue) {
    ...
    var question = "";
    if ("question" in queue[row]) {
      //Questions are not public
      question = queue[row].question;
    }

    if ("email" in queue[row]) {
      //TAs can see the user's email address
    ...
    var new_row = $(
      "<tr class='container'>" +
        ...
        i +
        "</td>" +
        "<td class='col-sm-2' align='left' style='word-wrap:break-word'>" +
        full_name +
        "</td>" +
        "<td class='col-sm-2' align='left' style='word-wrap:break-word'>" +
        Location +
    ...
    if (username in helping) {
      ...
      if (time_lim > 0) {
        ...
        duration =
          parseInt(fields[0]) * 3600 +
          parseInt(fields[1]) * 60 +
          parseInt(fields[2]);
    ...
    if (is_TA) {
      // HELP BUTTON
      if (username in helping) {
        //Student is currently being helped
        if (my_username === helping[username]) {
          //The TA is currently helping a current student
          var help_button = $(
            '<div class="btn-group" role="group"><button class="btn btn-primary" title="Stop Helping"> <i class="fa fa-undo"></i>  </button></div>'
          );
          help_button.click(function (event) {
            release_ta(course_id);
          });
        } else {
          //The student is currently being helped, but not by this TA, so don't let them end the other TA's help session
          var help_button = $(
            '<div class="btn-group" role="group"><button class="btn btn-primary" title="Stop Helping" disabled=true> <i class="fa fa-undo"></i>  </button></div>'
          );
        }
      } else {
        //Student is not being helped
        var help_button = $(
          '<div class="btn-group" role="group"><button class="btn btn-primary" title="Help Student"><i class="glyphicon glyphicon-hand-left"></i></button></div>'
        );
        help_button.click(function (event) {
          //If a TA helps a user, but isn't on duty, put them on duty
      ...
      // MOVE UP BUTTON
      var increase_button = $(
        '<div class="btn-group col" role="group"><button class="btn btn-primary" title="Move Up"> <i class="fa fa-arrow-up"></i>  </button></div>'
      );
      if (i == 1) {
        increase_button = $(
          '<div class="btn-group col" role="group"><button class="btn btn-primary" title="Move Up" disabled=true> <i class="fa fa-arrow-up"></i>  </button></div>'
        );
      }
      ...
      // MOVE DOWN BUTTON
      var decrease_button = $(
        '<div class="btn-group col" role="group"><button class="btn btn-primary" title="Move Down"> <i class="fa fa-arrow-down"></i>  </button></div>'
      );
      if (i == dataParsed.queue_length) {
        decrease_button = $(
          '<div class="btn-group col" role="group"><button class="btn btn-primary" title="Move Down" disabled=true> <i class="fa fa-arrow-down"></i>  </button></div>'
        );
      }
      ...
      // REMOVE BUTTON
      var dequeue_button = $(
        '<div class="btn-group col" role="group"><button class="btn btn-primary" title="Remove"> <i class="fa fa-close"></i>  </button></div>'
      );
      ...
      var button_group = $(
        "<div class='btn-group btn-group-justified container' role='group' aria-label='...'></div>"
      );
      ...
      if (username === my_username) {
        // Only add the move down button if it's the user's row
        var decrease_button = $(
          '<div align="right"><button class="btn btn-primary" title="Move Down"> <i class="fa fa-arrow-down"></i>  </button></div>'
        );
        if (i == dataParsed.queue_length) {
          decrease_button = $(
            '<div align="right"><button class="btn btn-primary" disabled=true title="Move Down"> <i class="fa fa-arrow-down"></i>  </button></div>'
          );
        }
        decrease_button.click(function (event) {
...
function set_quest_public(course_id, quest_public) {
  var url = "../api/queue/" + course_id + "/settings";
  var posting = $.post(url, {
    setting: "quest_public",
    quest_public: quest_public,
  });
  posting.done(done);
  posting.fail(fail);
}
